...
@app.route('/drinks')
def get_all_drinks():
    all_drinks = Drink.query.order_by(desc(Drink.id)).all()
...
def get_all_drinks_detailed(payload):
    all_drinks_detailed = Drink.query.order_by(desc(Drink.id)).all()

    return jsonify({
        'success': True,
        'drinks': [drink.long() for drink in all_drinks_detailed]
